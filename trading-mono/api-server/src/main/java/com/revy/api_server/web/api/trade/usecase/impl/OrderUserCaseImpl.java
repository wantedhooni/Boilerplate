package com.revy.api_server.web.api.trade.usecase.impl;
...
import com.revy.api_server.web.api.trade.payload.OrderPayload;
import com.revy.api_server.web.api.trade.usecase.OrderUseCase;
import com.revy.api_server.web.exception.TradeException;
...
public class OrderUserCaseImpl implements OrderUseCase {
    ...
    public OrderPayload.Res createOrder(Long userId, OrderPayload.Req req) {
        ...
        Account account = accountHelper.findOneByOwnerIdAndAccountNo(userId, req.accountNo());
        ...
        if(req.type() == OrderType.LIMIT){
            account = accountHelper.updateBuyOrderAccountBalance(account, totalOrderPrice);
        }
    ...
    public OrderPayload.Res cancelOrder(UUID orderId) {
        ...
        order.cancel();
        Account account = accountHelper.findById(order.getAccountId());
        if(order.getType() == OrderType.LIMIT){
            BigDecimal totalOrderPrice = getTotalOrderPrice(order);
            accountHelper.updateCancelOrderAccountBalance(account, totalOrderPrice);
        }
